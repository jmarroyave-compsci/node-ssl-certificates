...
class Module{
    ...
    async getCertificateFromDomain(domain, options){
      ...
      return await this.getInfo(cert.pemEncoded, { includeChain: true })
    }

    async getCertificateFromFile(path){
      const cert = require('fs').readFileSync(path, 'ascii');
      return await this.getInfo(cert, { includeChain: true })
    }

    async getInfo(cert, params={}){
      const { includeChain=true } = params
      var info;
      try{
        info = this.getCertificateInfo(cert)
